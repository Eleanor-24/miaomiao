        ...
        <ul>
            <li v-for="item in cinemaList" :key="item.cinemaId">
                <div>
                    <span>{{item.name}}</span>
                    <span class="q">
                        <span class="price">￥{{item.lowPrice/100}}</span>
                        <span>起</span>
                    </span>
                </div>
                <div class="address">
                    <span>{{item.address}}</span>
                    <span>{{Math.ceil(item.Distance*100)}}km</span>
                </div>
...
export default {
    ...
    activated(){
        ...
        this.isLoading=true
        this.axios({
            url:"https://m.maizuo.com/gateway?cityId="+cityId+"&ticketFlag=1&k=526625",
            headers:{
...
.cinema_body .price{ font-size: 14px;}
